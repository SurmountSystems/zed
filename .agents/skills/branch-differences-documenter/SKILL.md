...
> NEVER INVENT DIFFERENCES: You must only describe differences that are explicitly visible in the exact diff text or file list from a command you ran or the human supplied. Never add, assume, extrapolate, or "plausibly complete" any change not present in the supplied input.
...
- After obtaining a high-level changed file list or new batch of diff output (run read-only git yourself or use human-supplied output), first propose a clean de-duplicated category structure that groups obviously similar changes and refactors. For any borderline grouping decision (things that look similar but may actually differ in important ways), include a `TODO: Consider splitting [these items] into separate categories because [specific distinction]. Human to confirm grouping.` note in the output.
- When working on a chunk or category, analyze only the exact diff text for it. Draft concise natural-language documentation entries that state what changed and the observable effect. Keep entries short and reviewable.
...
Workflow:
1. Start the process. Run or request the precise high-level read-only git command for the initial changed-file list (e.g. `git diff --name-only --diff-filter=ACMRT main...surmount | head -200`).
...
4. Pick the next remaining todo (human may override). Run or request the exact targeted read-only git command to pull the specific diff hunks for that category/chunk.
5. Analyze only the exact diff text for the current item.
...
Never:
- Run mutating or destructive git commands, or read-only git when not 100% confident it is safe.
- Simulate or assume the result of any git command.
- Run any broad search, tree walk, rg, grep, or request files beyond the exact diff output needed for the current chunk.
- Invent, assume, or fabricate any difference, intent, effect, or categorization not explicitly supported by the diff text.
...
Prioritize:
- Absolute fidelity to the exact diff text.
...
- Run read-only git autonomously when safe; otherwise ask the human before proceeding.