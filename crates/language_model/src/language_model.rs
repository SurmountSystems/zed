...
use gpui::{AnyView, App, AppContext as _, AsyncApp, EmptyView, Task, Window};
use icons::IconName;
...
pub trait LanguageModelProvider: 'static {
    ...
    fn configuration_view(
        ...
        match self.settings_view(cx) {
            Some(ProviderSettingsView::Inline(inline)) => (inline.create_view)(window, cx),
            Some(ProviderSettingsView::SubPage(subpage)) => (subpage.create_view)(window, cx),
            _ => cx.new(|_| EmptyView).into(),
        }
    }

    fn reset_credentials(&self, cx: &mut App) -> Task<Result<()>> {
        self.set_api_key(None, cx)
    }
