...
**Grok Build native + bridging (multi-root bundled skills):** We scan three global roots (`.agents/skills/`, `~/.grok/skills/`, `~/.grok/bundled/skills/`) plus two project-local (`.agents/skills/`, `.grok/skills/`). `.agents` roots use `Global`/`ProjectLocal`; `.grok` roots use `GrokUser`/`GrokProjectLocal` (equal precedence within tier). Load order gives `.agents > .grok/user > bundled` (and agents project over grok project) via first-wins on ties. Bundled skills from the grok install are available; user `.grok` skills shadow them. `is_agents_skills_path`, gating, and fast-paths cover all. Loads parallel under concurrency. The distinct Grok* sources let native grok skills render with `<source>user</source>` / `<source>project</source>` in envelopes. See AGENTS.md multi-root bundled skills.
