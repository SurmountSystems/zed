...
impl ConversationView {
        ...
        match event {
            ...
            AcpThreadEvent::EntryUpdated(index) => {
                if let Some(active) = self.thread_view(&session_id) {
                    ...
                    active.update(cx, |active, cx| {
                        active.auto_expand_streaming_thought(window, cx);
                    });
                }
    ...
    fn dispatch_grok_completion_system_notification(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
...
pub(crate) mod tests {
    ...
    async fn test_thread_search_finds_matches_across_entries(cx: &mut TestAppContext) {
        ...
        bar.update_in(cx, |bar, window, cx| {
            bar.select_next_match(&crate::SelectNextThreadMatch, window, cx);
        });
        cx.run_until_parked();
        ...
        bar.update_in(cx, |bar, window, cx| {
            bar.select_prev_match(&crate::SelectPreviousThreadMatch, window, cx);
        });
    ...
    async fn test_thread_search_includes_expanded_thinking_blocks(cx: &mut TestAppContext) {
        ...
        // Surmount keeps thinking expansion on ThreadView (not EntryViewState).
        thread_view.update(cx, |view, _cx| {
            view.expanded_thinking_blocks
                .insert((assistant_entry_ix, thought_chunk_ix));
        });
        bar.update_in(cx, |bar, window, cx| {
            bar.update_matches(window, cx);
        });
    ...
    async fn test_thread_search_includes_expanded_tool_call_content(cx: &mut TestAppContext) {
        ...
        thread_view.update(cx, |view, _cx| {
            view.expanded_tool_calls.insert(tool_call_id);
        });
        bar.update_in(cx, |bar, window, cx| {
            bar.update_matches(window, cx);
        });
    ...
    #[gpui::test]
    async fn test_thread_search_includes_confirmation_tool_content(cx: &mut TestAppContext) {
        init_test(cx);

        let tool_call_id = acp::ToolCallId::new("confirm-search-content");
        let tool_call = acp::ToolCall::new(tool_call_id.clone(), "Needs approval")
            .kind(acp::ToolKind::Other)
            .content(vec!["Approval body mentions papaya once.".into()]);
        let connection =
            StubAgentConnection::new().with_permission_requests(HashMap::from_iter([(
                tool_call_id,
                PermissionOptions::Flat(vec![acp::PermissionOption::new(
        ...
        let thread =
            active_thread(&conversation_view, cx).read_with(cx, |view, _| view.thread.clone());
        thread
            .update(cx, |thread, cx| thread.send_raw("Please run this", cx))
            .await
            .unwrap();
        cx.run_until_parked();
        ...
        assert_eq!(
            bar.read_with(cx, |bar, _| bar.match_count()),
            1,
            "confirmation-forced-open tool body should be searchable without expand",
        );
    }
