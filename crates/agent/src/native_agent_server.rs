    ...
    agent_servers::e2e_tests::common_e2e_tests!(
        async |fs, cx| {
            let auth = cx.update(|cx| {
                ...
                let maybe_provider = registry.provider(&language_model::ANTHROPIC_PROVIDER_ID);

                match maybe_provider {
                    Some(provider) => {
                        let auth = provider.authenticate(cx);
                        cx.spawn(async move |_| auth.await)
                    }
                    None => {
                        panic!(
                            "Native agent e2e test requested Anthropic provider (for claude-sonnet-4-latest) but it was not present in the LanguageModelRegistry. \
                             These tests are generated by common_e2e_tests! and are now emitted with plain #[ignore = rich message] (no cfg hiding) so they are always discovered in normal test runs (preserving total count) but skipped unless --ignored or the 'e2e' feature. \
                             They require the 'e2e' feature + real Anthropic credentials and will now produce a clear failure instead of an unwrap when the provider is absent."
                        )
                    }
                }
            });
