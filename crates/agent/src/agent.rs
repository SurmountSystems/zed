...
use ::agent_skills::{
    AGENTS_DIR_NAME, MAX_SKILL_FILE_SIZE, ProjectSkillGroup, SKILL_FILE_NAME, SkillIndex,
    parse_skill_frontmatter, read_skill_body_from_content,
};
...
impl NativeAgent {
    ...
    fn publish_skill_index(&self, cx: &mut Context<Self>) {
        ...
        for state in self.projects.values() {
            for skill in state.skills.iter() {
                match &skill.source {
                    SkillSource::BuiltIn => {}
                    SkillSource::Global | SkillSource::GrokUser => {
                        if !seen_global {
    ...
    fn send_skill_invocation(
        ...
        let (path_style, project_for_spawn) = {
            let Some(state) = self.session_project_state(&session_id) else {
                return Task::ready(Err(anyhow!("Project state not found for session")));
            };
            (state.project.read(cx).path_style(cx), state.project.clone())
        };
        let fs_for_spawn = self.fs.clone();

        cx.spawn(async move |this, cx| {
            let fs_for_resolver = fs_for_spawn.clone();
            let _read_skill_body =
                skill_body_resolver_for_project(project_for_spawn, fs_for_resolver);
            let (acp_thread, thread) = this.update(cx, |this, _cx| {
                let session = this
                    .sessions
                    .get(&session_id)
            ...
            let body = if let Some(embedded) = skill.embedded_body {
                ...
                agent_skills::read_skill_body_for_native_agent(
                    fs_for_spawn.as_ref(),
...
pub fn skill_body_resolver_for_project(
    ...
    move |skill, cx| match skill.source.clone() {
        ...
        | SkillSource::GrokProjectLocal { worktree_id, .. } => {
            let project = project.clone();
            cx.spawn(async move |cx| {
        ...
        SkillSource::BuiltIn | SkillSource::Global | SkillSource::GrokUser => {
            let fs = fs.clone();
            cx.background_spawn(async move {
