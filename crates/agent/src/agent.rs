        ...
        cx.spawn(async move |cx| {
            ...
            if let Some(thread) = acp_thread.upgrade() {
                if let Ok(term_entity) = thread.update(cx, |t, _| t.terminal(terminal_id.clone())) {
                    let (resp, exit_status, pending_output) = thread.update(cx, |t, cx| {
                        let resp = term_entity.update(cx, |term, c| term.current_output(c));
                        let exit_status = resp
                            .exit_status
                            .clone()
                            .or_else(|| t.terminal_exit_status(&terminal_id));
                        let pending_output = t.pending_terminal_output(&terminal_id);
                        (resp, exit_status, pending_output)
                    });
