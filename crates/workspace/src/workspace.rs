...
pub struct Toast {
    ...
    action_button_style: Option<ui::ButtonStyle>,
    action_button_border_color: Option<gpui::Hsla>,
}
...
impl Toast {
    pub fn new<I: Into<Cow<'static, str>>>(id: NotificationId, message: I) -> Self {
        Toast {
            ...
            action_button_border_color: None,
        }
    }

    pub fn action_button_style(mut self, style: ui::ButtonStyle) -> Self {
        self.action_button_style = Some(style);
        self
    }

    pub fn action_button_border_color(mut self, color: gpui::Hsla) -> Self {
        self.action_button_border_color = Some(color);
        self
    }
...
impl Workspace {
    ...
    fn render_notifications(&self, _window: &mut Window, _cx: &mut Context<Self>) -> Option<Div> {
        if self.notifications.is_empty() {
            ...
            let merge_review_toast = |id: &NotificationId| {
                matches!(
                    id,
                    NotificationId::Named(name) if name.as_ref() == "surmount-merge-review"
                )
            };
            let (top_notifications, bottom_notifications): (Vec<_>, Vec<_>) = self
                .notifications
                .iter()
                .partition(|(id, _)| merge_review_toast(id));
            Some(
                div()
                    .absolute()
                    .right_3()
                    ...
                    .flex_col()
                    .justify_between()
                    .when(!top_notifications.is_empty(), |parent| {
                        parent.child(
                            div()
                                .w_112()
                                .flex()
                    ...
                    .when(!bottom_notifications.is_empty(), |parent| {
                        parent.child(
                            div()
                                .w_112()
                                .flex()
