        ...
        self.show_notification(toast.id.clone(), cx, |cx| {
            cx.new(|cx| {
                let mut notification =
                    simple_message_notification::MessageNotification::new(toast.message, cx);
                if let Some((click_msg, on_click)) = toast.on_click {
                    notification = notification
                        .primary_message(click_msg)
                        .primary_on_click(move |window, cx| on_click(window, cx));
                }
                if let Some(style) = toast.action_button_style {
                    notification = notification.button_style(style);
                }
                if let Some(color) = toast.action_button_border_color {
                    notification = notification.primary_button_border_color(color);
                }
                notification
            })
        });
...
pub mod simple_message_notification {
    ...
    pub struct MessageNotification {
        ...
        primary_button_border_color: Option<gpui::Hsla>,
    ...
    impl MessageNotification {
            ...
            F: 'static + Fn(&mut Window, &mut Context<Self>) -> AnyElement,
        {
            Self {
                ...
                primary_button_border_color: None,
        ...
        pub fn primary_button_border_color(mut self, color: gpui::Hsla) -> Self {
            self.primary_button_border_color = Some(color);
            self
        }
        ...
        fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
            ...
            let suffix = h_flex()
                ...
                .children(self.primary_message.iter().map(|message| {
                    ...
                    if let Some(color) = self.primary_button_border_color {
                        div()
                            .border_1()
                            .border_color(color)
                            .child(button)
