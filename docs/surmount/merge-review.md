...
> **Status:** Branch Diff workflow, heed3 session persistence (queue cursor, branch-diff selection, git SHA sync, UI state), and toast placement are in tree. Headless dogfood: the **agent** runs `cargo xtask dogfood merge-review` (Surmount root, trusted user-data, Start → chrome expects → Preview → End unless `--start-only`). **Default PreMerge green** on clean trees (no conflict chrome without `MERGE_HEAD` — by design). **R3 Decide hard spine** (`--with-conflict`): chrome → Review Diff → Summarizing → **synthetic Summary capture** through production parser (hard L2 = capture log, not rail alone) → Discuss/Record + resolve (soft L3 = product evidence). Optional `--decide-live-agent` soft-polls live Grok (settle ok → skip synthetic; soft-skip → synthetic when settle fails) — **hard green never requires Grok**. Operator skill: [`.agents/skills/zed-dogfood/SKILL.md`](../../.agents/skills/zed-dogfood/SKILL.md); in-Zed review: [`.agents/skills/surmount-merge-review/SKILL.md`](../../.agents/skills/surmount-merge-review/SKILL.md); plan: [`plans/0_agentic_dogfooding.md`](../../plans/0_agentic_dogfooding.md).
