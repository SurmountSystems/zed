...
### Grok Build (xAI) {#grok-build-xai}

Run the full [Grok Build](https://x.ai/cli) agent inside Zed's [agent panel](./agent-panel.md) over ACP. You get Grok's plan mode, skills, background tasks, and MCP with Zed's diff, terminal, and thread UI.

Sub-agents are available but often burn tokens without clear benefit. Prefer working in a single main thread unless you have a specific reason to delegate.

#### Installation

Install Grok Build with the [official script](https://docs.x.ai/docs/grok-build), then add it to your settings:
...
On Linux, Zed synthesizes this default if `~/.grok/bin/grok` exists — no settings entry required.

`XAI_API_KEY` and other `GROK_*` environment variables are forwarded from your shell or Zed's xAI keychain entry.

#### Getting Started

Open the agent panel, then start a Grok thread:

- {#kb agent::NewGrokThread} (`ctrl-alt-x` on Linux/Windows, `cmd-alt-x` on macOS)
- The agent selector or `+` menu → **grok**
- Command Palette → **agent: new grok thread**

When the binary is present, the thread header shows a **Grok Build (Co-Equal)** chip. Hover it for setup tips.

See the [Grok Build docs](https://docs.x.ai/docs/grok-build) for CLI features — they render in Zed when bridged over ACP.

#### Full Agent Mode

The default thread view shows a compact activity bar. **Full Agent Mode** opens the full categorized todos surface: pending tool approvals, plan items, background monitors, and Grok memory.

Ways to open it:

- Click **Full Agent Mode** in the thread toolbar
- Command Palette → **agent: open full grok surface** or **agent: open zed todos surface**
- Thread menu → **Full Grok Surface**
- `ctrl-alt-shift-t` (Linux) or `cmd-alt-shift-t` (macOS)

On Windows, use the toolbar button, menu, or Command Palette — there is no dedicated keybinding yet.

#### Plan Mode and Approvals

Grok Build uses `enter_plan_mode` and `todo_write` for planning. When a plan is proposed, Zed shows a **Plan proposed** banner with an accept button. Tool calls that need your OK appear in **Agent Approvals** with read-only vs destructive labels.

Approve or deny from the activity bar or Full Agent Mode surface. Background commands started with `monitor` show up under **Background Monitors**; expand one to watch live output.

#### Skills

Grok threads load skills from:

- `.agents/skills/` (project and global)
- `~/.grok/skills/` and `~/.grok/bundled/skills/`

Project `.agents` skills take precedence over project `.grok` skills. See [Skills](./skills.md).

#### Native Grok vs Bridged ACP

Two paths share the same Zed UI:

- **Bridged ACP** — the official Grok Build binary over stdio (configuration above)
- **Native** — xAI Grok models in Zed's built-in agent with the Grok Build profile

Both surface plans, approvals, monitors, and memory in the categorized todos UI.
