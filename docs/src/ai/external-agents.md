...
Inside Zed a minimal discoverability affordance exists for bridged Grok users: the global action "agent: open zed todos surface" (registered on every workspace via the agent panel init). Dispatching it (from command palette or keybind) focuses the agent panel and overlays a dedicated ZT-1 surface implemented by instantiating ZedTodosDockPrototype with the active thread weak (thin wrapper in the panel). The overlay reuses the exact classified rendering, collectors, risk chips, actions, and expansion state of the prototype while the toolbar and back/close behaviors remain. This delivers low-friction native access to the full persistent ZT-1 surface without TUI popups or full conversation context. Users can bind the action for daily use. All changes limited to existing files; follows the reusable guarantee.
